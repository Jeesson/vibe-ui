...
const props = withDefaults(defineProps<{ modelValue: File[]; multiple?: boolean; accept?: string }>(), {
    multiple: true,
});
        ...
        <ul v-if="modelValue.length" class="mt-2 flex flex-col gap-1">
            <li
                ...
                <span class="flex items-center gap-2 text-gray-400">
                    ...
                    <button type="button" class="hover:text-red-500" @click="removeFile(i)">✕</button>
                </span>
            </li>
        </ul>
    </div>
