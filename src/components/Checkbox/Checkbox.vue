...
const checked = computed(() => (group && props.value !== undefined ? group.isChecked(props.value) : props.modelValue));
...
<template>
    <label
        ...
        <input type="checkbox" class="sr-only" :checked="checked" :disabled="disabled" @change="onChange" />
        <span
            class="flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors"
            :class="checked ? 'bg-primary-600 border-primary-600 text-white' : 'border-gray-300 bg-white'">
            <svg v-if="checked" viewBox="0 0 12 12" class="h-2.5 w-2.5" fill="none">
